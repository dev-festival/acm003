...
# Merge needs with has monitoring
coverage_report = has_monitoring.merge(
    assets_with_needs[['ASSETNUM'] + [col for col in assets_with_needs.columns if col.startswith('NEEDS_')]],
    on='ASSETNUM',
    how='outer'
)
...
for col in coverage_report.columns:
    if col.startswith('NEEDS_') or col.endswith('_HAS'):
...
for tech in tech_cols:
    ...
    has_col = f'HAS_{tech}'  # Consistent naming for all technologies
    judge_col = f'{tech}_JUDGE'
    
    # Apply judgment logic
    # 0 = Critical gap (Primary need not met)
    # 1 = Acceptable (all other cases)
    coverage_report[judge_col] = coverage_report.apply(
        lambda row: 0 if (row[needs_col] == 'P' and row[has_col] == 'N') else 1,
        axis=1
    )

# Create MASTER_JUDGE: 0 if ANY technology has a critical gap, else 1
judge_cols = [f'{tech}_JUDGE' for tech in tech_cols]
coverage_report['MASTER_JUDGE'] = coverage_report[judge_cols].min(axis=1)
