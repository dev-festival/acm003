...
for tech in tech_cols:
    ...
    # Apply tri-state judgment logic
    # 0 = Critical gap (Primary need not met)
    # 1 = Compliant (Primary/Secondary need met)
    # 2 = Not Required (doesn't need this technology)
    coverage_report[judge_col] = coverage_report.apply(
        lambda row: (
            0 if (row[needs_col] == 'P' and row[has_col] == 'N') else  # Critical gap
            1 if (row[needs_col] in ['P', 'S'] and row[has_col] == 'Y') else  # Compliant
            2  # Not Required (includes Secondary needs not met)
        ),
        axis=1
    )

# Create MASTER_JUDGE column (tri-state)
# 0 = At least one critical gap (any tech has JUDGE = 0)
# 1 = All needs met (all techs are either 1 or 2, with at least one 1)
# 2 = Not required for ANY technology (all techs are 2)

judge_cols = [f'{tech}_JUDGE' for tech in tech_cols]

coverage_report['MASTER_JUDGE'] = coverage_report[judge_cols].apply(
    lambda row: (
        0 if (row == 0).any() else          # Any gap = Critical
        2 if (row == 2).all() else          # All not required = Not Required
        1                                    # Otherwise = Compliant
    ),
    axis=1
)
...
print(f"Assets with FULL compliance (MASTER_JUDGE = 1): {(coverage_report['MASTER_JUDGE'] == 1).sum():,}")
print(f"Assets with critical gaps (MASTER_JUDGE = 0): {(coverage_report['MASTER_JUDGE'] == 0).sum():,}")
print(f"\nCritical gaps by technology (Primary need not met):")
for tech in tech_cols:
    gap_count = (coverage_report[f'{tech}_JUDGE'] == 0).sum()
    if gap_count > 0:
        print(f"  {tech}: {gap_count:,} assets")
